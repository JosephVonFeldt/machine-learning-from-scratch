...
void saveLayer(NNLayer *layer, int preNodeCount, FILE *fptr) {
    ...
    // Biases
    for(int col =0; col < layer->nodeCount; col++) {
        fprintf(fptr, "%f ", layer->biases->values[col]);
    }
    fprintf(fptr, "]");
}
    ...
    for (int i = 0; i <= nn->numHiddenLayers; i++) {
        ...
        for(int rowIndex = 0; rowIndex < rows; rowIndex++) {
            for(int colIndex = 0; colIndex < cols; colIndex++){
                fscanf(fptr, " %lf ", &val);
                currLayer->weights->values[rowIndex * cols + colIndex] = val;
                if (rowIndex < 2) {
                    printf("%i\t",rowIndex * cols + colIndex);
                    printf("%f\t%lf\n", val,currLayer->weights->values[rowIndex * rows + colIndex]);
                }
        ...
        for(int colIndex = 0; colIndex < cols; colIndex++){
            fscanf(fptr, " %lf", &val);
            //printf("%f\n", val);
            currLayer->biases->values[colIndex] = val;
        }
        fscanf_s(fptr, "%s", w);
